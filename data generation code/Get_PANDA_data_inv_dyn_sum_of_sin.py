...
p.add_argument('-saving_path',
               type=str,
               default='./data/data_no_noise/',
               help='saving folder')
p.add_argument('-seed',
               type=int,
               default=0,
               help='seed')
p.add_argument('-sim_interval',
               type=int,
               default=50,
               help='sim_interval')
p.add_argument('-pos_range',
               type=float,
               default=1.,
               help='pos_range percentage')
p.add_argument('-w_f',
               type=float,
               default=0.02,
               help='Fundamental pulsation')
p.add_argument('-num_sin',
               type=int,
               default=5,
               help='number of sinusoids')
p.add_argument('-c_init',
               type=float,
               default=1.,
               help='initial range of the sinusoids coefficient distribution')
p.add_argument('-flg_frict',
               type=bool,
               default=False,
               help='if true use friction')
p.add_argument('-p_dyn_perc_error',
               type=float,
               default=0.,
               help='percentage multiplicative error')
p.add_argument('-p_dyn_add_std',
               type=float,
               default=0.,
               help='std additive error')
p.add_argument('-sampling_time',
               type=float,
               default=0.001,
               help='number of non fixed joints -- to simulate lower dof configurations ')
p.add_argument('-num_active_joints',
               type=int,
               default=7,
               help='number of non fixed joints -- to simulate lower dof configurations ')
p.add_argument('-std_noise',
               type=float,
               default=0.,
               help='torque noise std')
p.add_argument('-plot',
               type=int,
               default=1,
               help='if 1 plot results')
...
def get_sum_of_sin_trj(num_dof, q_ranges, dq_ranges, ddq_ranges, w_f, num_sin, c_init, T_sampling, sim_interval, num_active_joints=-1):
    """
        ...
        if joint_index < num_active_joints:
            ...
            while not(flg_feasible):
                ...
                for sin_index in range(num_sin):
                    l = sin_index+1
                    pos = pos + a_k[sin_index]/(w_f*l)*np.sin(w_f*l*t) - b_k[sin_index]/(w_f*l)*np.cos(w_f*l*t)
                    vel = vel + a_k[sin_index]*np.cos(w_f*l*t) + b_k[sin_index]*np.sin(w_f*l*t)
                    acc = acc - a_k[sin_index]*(w_f*l)*np.sin(w_f*l*t) + b_k[sin_index]*(w_f*l)*np.cos(w_f*l*t)
...
def get_tau(q, q_dot, q_ddot, damping_coef, friction_coef, offset_coef):
    ...
    for t in range(0, num_samples):
        ...
        tau[t,:] = inv_dyn(p_dyn, q[t,:], q_dot[t,:], q_ddot[t,:]) + damping_coef*q_dot[t,:] + friction_coef*np.sign(q_dot[t,:]) + offset_coef           
    return tau
...
def get_phi(q, q_dot, q_ddot):
    ...
    for t in range(0, num_samples):
        phi[t,:] = np.array(H.H(q[t,:],
                              q_dot[t,:],
                              q_ddot[t,:])).reshape([1,-1])
    return phi
...
if p_dyn_perc_error==0. and p_dyn_add_std==0.:
    if flg_frict:
        saving_name_data = saving_path+'data_inv_dyn_sum_of_sin__noise_std'+str(std_noise)+'_friction__w_f'+str(w_f)+'__num_sin'+str(num_sin)+'__pos_range_'+str(pos_range)+'__'+str(sim_interval)+'seconds'+'__Ts_'+str(sampling_time)+'__seed'+str(seed)+'.pkl'
        ...
        saving_name_data_M = saving_path+'data_M_inv_dyn_sum_of_sin__noise_std'+str(std_noise)+'_friction__w_f'+str(w_f)+'__num_sin'+str(num_sin)+'__pos_range_'+str(pos_range)+'__'+str(sim_interval)+'seconds'+'__Ts_'+str(sampling_time)+'__seed'+str(seed)+'.pkl'
        ...
        saving_name_data = saving_path+'data_inv_dyn_sum_of_sin__noise_std'+str(std_noise)+'__w_f'+str(w_f)+'__num_sin'+str(num_sin)+'__pos_range_'+str(pos_range)+'__'+str(sim_interval)+'seconds'+'__Ts_'+str(sampling_time)+'__seed'+str(seed)+'.pkl'
        ...
        saving_name_data_M = saving_path+'data_M_inv_dyn_sum_of_sin__noise_std'+str(std_noise)+'__w_f'+str(w_f)+'__num_sin'+str(num_sin)+'__pos_range_'+str(pos_range)+'__'+str(sim_interval)+'seconds'+'__Ts_'+str(sampling_time)+'__seed'+str(seed)+'.pkl'
    ...
    if flg_frict:
        saving_name_data = saving_path+'data_inv_dyn_sum_of_sin__noise_std'+str(std_noise)+'_friction__p_dyn_add_std_'+str(p_dyn_add_std)+'__p_dyn_add_std_'+str(p_dyn_add_std)+'__w_f'+str(w_f)+'__num_sin'+str(num_sin)+'__pos_range_'+str(pos_range)+'__'+str(sim_interval)+'seconds'+'__Ts_'+str(sampling_time)+'__seed'+str(seed)+'.pkl'
        ...
        saving_name_data_M = saving_path+'data_M_inv_dyn_sum_of_sin__noise_std'+str(std_noise)+'_friction__p_dyn_add_std_'+str(p_dyn_add_std)+'__p_dyn_add_std_'+str(p_dyn_add_std)+'__w_f'+str(w_f)+'__num_sin'+str(num_sin)+'__pos_range_'+str(pos_range)+'__'+str(sim_interval)+'seconds'+'__Ts_'+str(sampling_time)+'__seed'+str(seed)+'.pkl'
        ...
        saving_name_data = saving_path+'data_inv_dyn_sum_of_sin__noise_std'+str(std_noise)+'__p_dyn_add_std_'+str(p_dyn_add_std)+'__p_dyn_add_std_'+str(p_dyn_add_std)+'__w_f'+str(w_f)+'__num_sin'+str(num_sin)+'__pos_range_'+str(pos_range)+'__'+str(sim_interval)+'seconds'+'__Ts_'+str(sampling_time)+'__seed'+str(seed)+'.pkl'
        ...
        saving_name_data_M = saving_path+'data_M_inv_dyn_sum_of_sin__noise_std'+str(std_noise)+'__p_dyn_add_std_'+str(p_dyn_add_std)+'__p_dyn_add_std_'+str(p_dyn_add_std)+'__w_f'+str(w_f)+'__num_sin'+str(num_sin)+'__pos_range_'+str(pos_range)+'__'+str(sim_interval)+'seconds'+'__Ts_'+str(sampling_time)+'__seed'+str(seed)+'.pkl'
...
# gen trj
q, q_dot, q_ddot = get_sum_of_sin_trj(num_dof, q_ranges, dq_ranges, ddq_ranges, omega_f, num_sin, c_init, T_sampling, sim_interval, num_active_joints=num_active_joints)
...
data = pd.DataFrame(data=np.concatenate([q[:,:num_active_joints], q_dot[:,:num_active_joints], q_ddot[:,:num_active_joints], tau[:,:num_active_joints],tau_noiseless[:,:num_active_joints], m[:,:num_active_joints], c[:,:num_active_joints], g[:,:num_active_joints], U], axis=1),
                       columns=data_names)
